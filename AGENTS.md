...
- `raspberry-pi-docker-deploy`: Deploy this repository to a Raspberry Pi with Docker, verify `.env.deploy` and the selected app env file, run the repository deploy scripts, and diagnose Raspberry Pi Docker, GHCR pull, port, or I2C issues such as unauthorized registry access, allocated ports, missing `/dev/i2c-1`, or failed `i2cdetect` results. Use when the user asks to deploy, redeploy, or debug Raspberry Pi execution. (file: `./skills/raspberry-pi-docker-deploy/SKILL.md`)
